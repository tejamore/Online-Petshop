...
if (isset($_SESSION['user_users_id']) && $_SESSION['user_users_username'] !== null) {
    ...
    $insert_orders = "INSERT INTO petsworld_orders (users_id, delivery_date, payment_method, total_amount) values ('$users_id', '$delivery_date', '$payment_method', '$total_amount')";
    mysqli_query($conn, $insert_orders);
    $orders_id = mysqli_insert_id($conn);
    for ($i=0; $i < count($product_name); $i++) { 
    	$insert_orders_detail = "INSERT INTO petsworld_orders_detail (orders_id, product_name, quantity) values ('$orders_id', '$product_name[$i]', '$quantity[$i]')";
